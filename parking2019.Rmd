...
```{r}
ggplot(parking2019_plot2) +
  geom_violin(
    aes(x = arr_day, y = arr_dep_time, fill = arr_dep),
    scale = "count",
    alpha = 0.6,
    draw_quantiles = 0.5
  ) +
  facet_wrap(~ BetweenStreet1, nrow = 3) +
  scale_fill_manual(values = c("#6495ed", "#ed6495")) +
...
```{r}
ggplot(parking2019_plot2) +
  geom_boxplot(aes(x = arr_day, y = arr_dep_time, fill = arr_dep), alpha = 0.6) +
  facet_wrap( ~ BetweenStreet1, nrow = 3) +
  scale_fill_manual(values = c("#6495ed", "#ed6495"))
```
